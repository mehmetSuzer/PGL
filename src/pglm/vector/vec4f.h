...
inline vec4f scale_vec4f(vec4f v, float scale) {
    return (vec4f){
        v.x * scale, 
        v.y * scale, 
        v.z * scale,
...
inline float dot_vec4f(vec4f v1, vec4f v2) {
    return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z + v1.w * v2.w;
}

inline float mag2_vec4f(vec4f v) {
    return dot_vec4f(v, v);
}

inline float mag_vec4f(vec4f v) {
...
inline bool epsilon_not_equal_vec4f(vec4f v1, vec4f v2, float epsilon) {

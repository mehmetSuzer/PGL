...
inline quat quat_from_euler_angles(const vec3f angles) {
    ...
    const vec3f c = (vec3f) {
        fast_cos(half_angles.x),
        fast_cos(half_angles.y),
        fast_cos(half_angles.z),
    };
    const vec3f s = (vec3f) {
        fast_sin(half_angles.x),
        fast_sin(half_angles.y),
        fast_sin(half_angles.z),
    };
...
inline quat quat_from_vec3f_angle(const vec3f v, const float radian) {
    ...
    const float c = fast_cos(half_radian);
    const float s = fast_sin(half_radian);
...
inline float norm_quat(const quat q) {
    return fast_square_root(norm2_quat(q));
}
...
inline bool epsilon_equal_quat(const quat q1, const quat q2, const float epsilon) {
...
inline bool epsilon_not_equal_quat(const quat q1, const quat q2, const float epsilon) {

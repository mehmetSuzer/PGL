...
inline vec3f scale_vec3f(vec3f v, float scale) {
    return (vec3f){
        v.x * scale, 
        v.y * scale, 
        v.z * scale,
...
inline float dot_vec3f(vec3f v1, vec3f v2) {
    return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
}

inline float mag2_vec3f(vec3f v) {
    return dot_vec3f(v, v);
}

inline float mag_vec3f(vec3f v) {
...
inline bool epsilon_not_equal_vec3f(vec3f v1, vec3f v2, float epsilon) {

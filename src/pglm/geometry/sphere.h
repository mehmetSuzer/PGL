...
typedef struct {
    vec3f center;
    f32 radius; // REQUIREMENT: must be positive
} sphere_t;
...
static inline bool sphere_intersects_ray(ray_t ray, sphere_t sphere, f32 near, f32 far, f32* distance_out) {
    const vec3f center_to_source = vec3f_sub(ray.source, sphere.center);
    const f32 dist2 = vec3f_mag2(center_to_source);
    const f32 dot = vec3f_dot(center_to_source, ray.direction);
